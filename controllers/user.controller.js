...
const login = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ msg: `Hubo un error al loguearse ${error.message}` });
  }
};
...
const updatePassword = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ msg: `The user password was successfully updated.` });
  } catch (error) {
    return res.status(500).json({ msg: error.message });
  }
};
...
const followingUser = async (req, res = response) => {
  ...
  try {
    ...
    if (myUser.following.includes(idToFollow)) {
      return res
        .status(404)
        .json({ msg: "This user is already following this user" });
    }
...
const confirmAccount = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ msg: `El usuario se ha confirmado correctamente.` });
  } catch (error) {
    return res.status(500).json({ msg: error.message });
  }
};
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({
        msg: `Hemos enviado un correo con las instrucciones para la actualizacion`,
      });
  } catch (error) {
    return res.status(500).json({ msg: error.message });
  }
